  ...
  context "`postgresql.conf` file" do
    context "using the `standard` template" do
      it "creates the file" do
        ...
        expect(chef_run).to(render_file(postgresql_template))
        expect(chef_run.template(postgresql_template)).to(notify("service[postgresql]").to(:restart))
      end
      ...
      context "using the `conf_custom` attribute" do
        ...
        it "creates the file" do
          ...
          expect(chef_run).to_not(render_file(postgresql_template).with_content(/^# PostgreSQL configuration file/))
          expect(chef_run.file(postgresql_template)).to(notify("service[postgresql]").to(:restart))
        end
      end
    end
  end
